...
var settings = (global.db.data.settings && global.db.data.settings[this.user.jid]) || { prefix: defaultPrefix }
let prefix
if (settings.prefix) {
...
const isROwner = global.owner.map((v) => (Array.isArray(v) ? v[0] : v).replace(/[^0-9]/g, '') + detectwhat).includes(m.sender)
const isOwner = isROwner || m.fromMe
const isMods = isOwner || global.mods.map((v) => (Array.isArray(v) ? v[0] : v).replace(/[^0-9]/g, '') + detectwhat).includes(m.sender)
//const isPrems = isROwner || global.prems.map(v => (Array.isArray(v) ? v[0] : v).replace(/[^0-9]/g, '') + detectwhat).includes(m.sender)
