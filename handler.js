...
const {proto, jidNormalizedUser, areJidsSameUser, generateWAMessageFromContent} = baileys
...
if (Object.keys(global.plugins || {}).length === 0) {
    console.log('[DEBUG] global.plugins is empty in handler!');
}
if (!chatUpdate) {
return
}
...
const botJid = (this.user && this.user.jid) || ''
var settings = (global.db.data.settings && botJid && global.db.data.settings[botJid]) || { prefix: defaultPrefix }
let prefix
if (settings.prefix) {
    if (Array.isArray(settings.prefix)) {
        prefix = new RegExp('^(' + settings.prefix.map((p) => String(p).replace(/[|\\{}()[\]^$+*.\-\^]/g, '\\$&')).join('|') + ')')
    } else if (typeof settings.prefix === 'string' && settings.prefix.includes(',')) {
        const prefixes = settings.prefix.split(',').map((p) => p.trim())
        prefix = new RegExp('^(' + prefixes.map((p) => String(p).replace(/[|\\{}()[\]^$+*.\-\^]/g, '\\$&')).join('|') + ')')
    } else if (settings.prefix === defaultPrefix) {
        prefix = new RegExp('^[' + String(settings.prefix).replace(/[|\\{}()[\]^$+*.\-\^]/g, '\\$&') + ']')
    } else {
        prefix = new RegExp('^' + String(settings.prefix).replace(/[|\\{}()[\]^$+*.\-\^]/g, '\\$&'))
    }
} else {
...
// console.log('[DEBUG] global.owner:', JSON.stringify(global.owner));
const isROwner = global.owner.map((v) => {
    try {
        return String(Array.isArray(v) ? v[0] : v).replace(/[^0-9]/g, '') + detectwhat;
    } catch (e) {
        console.error(`[DEBUG] Error in isROwner map for v: ${v}`, e);
        return "";
    }
}).includes(m.sender)
...
const isMods = isOwner || global.mods.map((v) => {
    try {
        ...
        console.error(`[DEBUG] Error in isMods map for v: ${v}`, e);
        return "";
    }
}).includes(m.sender)
//const isPrems = isROwner || global.prems.map(v => String(Array.isArray(v) ? v[0] : v).replace(/[^0-9]/g, '') + detectwhat).includes(m.sender)
...
if (opts['nyimak']) {
    console.log('[DEBUG] nyimak is ON, returning');
    return
}
if (!isROwner && opts['self']) {
    console.log('[DEBUG] self mode is ON and not ROwner, returning');
    return
}
if (opts['pconly'] && m.chat.endsWith('g.us')) {
    console.log('[DEBUG] pconly is ON and is group, returning');
    return
}
if (opts['gconly'] && !m.chat.endsWith('g.us')) {
    console.log('[DEBUG] gconly is ON and not group, returning');
    return
}
if (opts['swonly'] && m.chat !== 'status@broadcast') {
     console.log('[DEBUG] swonly is ON and not status, returning');
     return
}
...
const ___dirname = path.join(process.cwd(), process.env.NODE_ENV === 'production' ? './dist/plugins' : './plugins')

console.log(`[DEBUG] Entering plugin loop. Total plugins: ${Object.keys(global.plugins || {}).length}`);
...
if (match) console.log(`[DEBUG] Plugin ${name} matched! match[0]: ${match[0]}`);
...
let fail = plugin.fail || global.dfail
    ...
    if (!isAccept) {
        // console.log(`[DEBUG] Plugin ${name} rejected command: ${command}. plugin.command: ${plugin.command}`);
        continue
    }
m.plugin = name
...
if (db.data.chats[m.chat] && db.data.chats[m.chat].reaction && m.text && m.text.match(/(ción|dad|aje|oso|izar|mente|pero|tion|age|ous|ate|and|but|ify)/gi)) {
let emot = pickRandom([
'😀',
'😃',
'😄',
...
.replace('@desc', groupMetadata.desc?.toString() || '🤖 𝗦𝘂𝗽𝗲𝗿 𝗲𝗹𝗹𝗕𝗼𝘁-𝗠𝗞 🤖')
...
thumbnail: apii.data,
title: [wm, '🤖 𝗦𝘂𝗽𝗲𝗿 ' + gt + ' 🤖', '🌟 ellbotmk@gmail.com'].getRandom(),
containsAutoReply: true,
mediaType: 1,
sourceUrl: 'https://github.com/balah6/ellBot-MK'
}
}
},
{quoted: fkontak2}
...
body: [wm, '🤖 𝗦𝘂𝗽𝗲𝗿 ' + gt + ' 🤖', '🌟 ellbotmk@gmail.com'].getRandom(),
thumbnail: gataImg,
sourceUrl: accountsgb
}
}
...
if (process.env.NODE_ENV !== 'production') {
    const file = (typeof import.meta !== 'undefined' && import.meta.url) ? (global.__filename ? global.__filename(import.meta.url, true) : fileURLToPath(import.meta.url)) : (typeof __filename !== 'undefined' ? __filename : '');
    if (file) {
