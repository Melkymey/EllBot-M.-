...
if (settings.prefix) {
    if (Array.isArray(settings.prefix)) {
        prefix = new RegExp('^(' + settings.prefix.map((p) => p.replace(/[|\\{}()[\]^$+*.\-\^]/g, '\\$&')).join('|') + ')')
...
if (db.data.chats[m.chat].reaction && m.text && m.text.match(/(ción|dad|aje|oso|izar|mente|pero|tion|age|ous|ate|and|but|ify)/gi)) {
let emot = pickRandom([
'😀',
'😃',
'😄',
