...
const isROwner = global.owner.map((v) => String(Array.isArray(v) ? v[0] : v).replace(/[^0-9]/g, '') + detectwhat).includes(m.sender)
const isOwner = isROwner || m.fromMe
const isMods = isOwner || global.mods.map((v) => String(Array.isArray(v) ? v[0] : v).replace(/[^0-9]/g, '') + detectwhat).includes(m.sender)
...
if (!isROwner && opts['self'] === true) return 
